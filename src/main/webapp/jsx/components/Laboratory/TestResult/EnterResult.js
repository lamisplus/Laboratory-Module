...
const ModalSampleResult = (props) => {
  ...
  const validate = () => {
    ...
    temp.date_result_reported = otherfields.date_result_reported
      ? ""
      : "Date  is required.";
    //temp.result_reported_by = otherfields.result_reported_by ? "" : "This filed is required."
    temp.date_asseyed = otherfields.date_asseyed
      ? ""
  ...
  const saveSample = async (e) => {
    ...
    if (validate()) {
      ...
      let resultSamples = {
        ...
        dateResultReported:
          otherfields.date_result_reported.replace("T", " ") + ":00",
        dateSampleReceivedAtPcrLab:
          otherfields.dateSampleReceivedAtPcrLab.length !== 0
            ? otherfields.dateSampleReceivedAtPcrLab.replace("T", " ") + ":00"
            : "",
        facilityId: 0,
        id: 0,
        patientId: patientId,
      ...
      try {
        ...
        // Get the visitId from the lab order data
        const visitId = datasample.visitId;

        console.log(
          "Attempting patient checkout with visitId:",
          visitId,
          "from datasample:",
          datasample
        );

        // Silently checkout the patient if visitId is available
        if (visitId) {
          try {
            await axios.put(
              `${url}patient/visit/checkout/${visitId}`,
              visitId,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            ...
            console.log(
              "Patient automatically checked out from lab visit:",
              visitId
            );
          } catch (checkoutError) {
            console.warn(
              "Failed to checkout patient automatically:",
              checkoutError
            );
            // Don't show error to user as this is a silent operation
          }
        } else {
          console.warn("No visitId available for patient checkout");
        }

        props.togglestatus();
        props.handDataReload();
      } catch (error) {
        console.error("Error saving sample result:", error);
        setLoading(false);
        toast.error("Failed to save sample result. Please try again.", {
          position: toast.POSITION.TOP_RIGHT,
        });
      }
            ...
            <Card>
              <CardBody>
                <Row style={{ marginTop: "20px" }}>
                  ...
                  <Col xs="6">
                    <FormGroup>
                      <Label for="date_asseyed" className={classes.label}>
                        Date Assayed
                      </Label>
                      <Input
                        type="datetime-local"
                ...
                <Row>
                  <Col md={6}>
                    <FormGroup>
                      <Label for="result_reported_by" className={classes.label}>
                        Reported by{" "}
                      </Label>
                  ...
                  {inputFlip === 0 ? (
                    <Col md={6}>
                      <FormGroup>
                        <Label for="test_result" className={classes.label}>
                          Result
                        </Label>
                ...
                <Row>
                  <Col md={12}>
                    <FormGroup>
                      <Label for="resultReported" className={classes.label}>
                        Result Report
                      </Label>
                      <Input
                        className={classes.input}
